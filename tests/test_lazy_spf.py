...
""")
        ...
        assert len(parser.mtrl_spf_fls) == 0, (
            f"Expected 0 materialized, got {len(parser.mtrl_spf_fls)}"
        )
        ...
        assert len(parser.mtrl_spf_fls) == 1, (
            f"Expected 1 materialized, got {len(parser.mtrl_spf_fls)}"
        )
...
""")
        ...
        assert spf_a in parser.mtrl_spf_fls, (
            f"Expected spf_a to be materialized, got {parser.mtrl_spf_fls}"
        )
...
def test_lazy_trace_json_eager_flush():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            "cell_json" not in parser.pndg_spf_fls
            or parser.subckts["cell_json"].is_placeholder is False
        )
...
""")
        ...
        assert len(back_annot_log) == 0, (
            f"Unexpected back-annotation during parse: {back_annot_log}"
        )
